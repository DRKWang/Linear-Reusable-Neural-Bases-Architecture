...
class CausalSelfAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, seq_length, dropout):
        super().__init__()
        ...
        T= seq_length
        self.k1 = nn.Parameter(torch.randn(embed_dim, self.head_dim))
        self.k2 = nn.Parameter(torch.zeros(self.head_dim, T))
        self.v = nn.Parameter(torch.zeros(embed_dim, embed_dim))
        # self.attn = nn.Parameter(torch.zeros(T, T))

    def forward(self, x, mask=None):
        B, T, C = x.shape

        attn = x @ (self.k1 @ self.k2)
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, ff_hidden_dim, dropout, activation, seq_length):
        super().__init__()
